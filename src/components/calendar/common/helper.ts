...
export default function generateCalendarGroup(
  ...
  return function (
    ...
    // 折叠成显示一周
    if (collapse) {
      const firstDateDay = date.day();
      // 获取当前周的前几天
      for (let i = 1; i <= firstDateDay; i++) {
        const thisDate = date.subtract(i, 'day').startOf('day');
        let item = {
          marks: [],
          _value: thisDate,
          text: thisDate.date(),
          type: getTypeOfMonth(thisDate, date),
...
export function getTypeOfMonth(date: dayjs.Dayjs, contrastMonthDate: dayjs.Dayjs): number {
  const month = date.month();
  const contrastMonth = contrastMonthDate.month();
  if(month < contrastMonth) {
    return constant.TYPE_PRE_MONTH;
  }
  else if (month > contrastMonth) {
    return constant.TYPE_NEXT_MONTH;
  }
  else {
    return constant.TYPE_NOW_MONTH;
  }
}
